...
public static class ProductApi {
    internal static RouteGroupBuilder MapProductApi(this RouteGroupBuilder group) {
        group.MapGet("/",
            async (IMediator md, [DefaultValue(1)] int? pageIndex, [DefaultValue(10)] int? pageSize) =>
                await md.Send(new ListProductsQuery(pageIndex ?? 1, pageSize ?? 10, typeof(ProductDto))));
        group.MapGet("/{productCode}",
            async (IMediator md, string productCode, IDistributedCache cache) => {
